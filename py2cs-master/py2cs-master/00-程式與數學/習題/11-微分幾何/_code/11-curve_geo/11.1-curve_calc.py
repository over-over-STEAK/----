...
def calculate_curvature_tensors(coords, metric):
    """
    計算給定度量張量的 Christoffel 符號、Riemann 張量、Ricci 張量、純量曲率和截面曲率。
    ...
    dim = len(coords)
    if dim != 2:
        print("警告: 截面曲率計算目前只適用於 2 維流形 (高斯曲率)。")
    ...
    # 轉換為協變形式 R_klij = g_km R^m_lij
    R_cov = sp.simplify(R.lowerindex(0))
    
    # 計算唯一的獨立分量 (R_theta_phi_theta_phi)
    # 座標索引: theta=0, phi=1
    riemann_component_0101 = R_cov[0, 1, 0, 1]
    
    print(f"唯一獨立協變分量 R_{coords[0], coords[1], coords[0], coords[1]} = {riemann_component_0101}")
    ...
    # 5. 截面曲率 (Sectional Curvature) K
    print("\n--- 5. 截面曲率 (Sectional Curvature) K (等同於高斯曲率) ---")
    
    # 2D 高斯曲率 K = R_0101 / det(g_ij)
    metric_det = sp.simplify(metric.det())
    
    if metric_det == 0:
        K_sectional = sp.nan # 奇異度量
    else:
        # 使用 R_0101 / det(g)
        K_sectional = sp.simplify(riemann_component_0101 / metric_det)

    print(f"截面曲率 K = {K_sectional}")
    ...
    return {
        ...
        "SectionalCurvature": K_sectional
    }
