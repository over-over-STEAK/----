...
def is_probably_shell_command(text: str) -> bool:
    ...
    if not t:
        return False
    # obvious shell operators
    if any(op in t for op in ["&&", "||", "|", ">", "<", ";"]):
        return True
    # first token command allowlist (conservative)
    try:
...
def run_shell(cmd: str, cwd: Path, timeout: int = 30) -> str:
    try:
        ...
        # Human-friendly mirror output (keep JSON too)
...
def run_llm_loop(task: str, model: str, cwd: Path, timeout: int, max_steps: int, mode: str = "auto", trace: bool = False) -> str:
    ...
    for step in range(1, max_steps + 1):
        ...
        # In tool mode, require actionable XML (<shell> or <final>) to avoid endless chat loops.
        ...
        # Execute actionable blocks first
        ...
        # If model says done, finish after execution of actionable blocks
        finals = [b for b in blocks if b.tag == "final"]
        if finals:
            final_body = finals[-1].body.strip()

            # 1) If there was tool execution in this step, keep structured output
            if runtime_output:
                out = [
            ...
            # 2) No tool execution: treat final as direct reply
            #    - shell-like command => auto execute
            #    - otherwise => plain markdown answer (no json wrapper)
...
def repl(cwd: Path, timeout: int, model: str, max_steps: int, mode: str, output_mode: str, trace: bool) -> int:
    ...
    while True:
        ...
        # Auto mode:
        # - has XML blocks -> treat as raw protocol message
        # - otherwise    -> treat as natural-language task and run llm loop
